...
*A program for backing up photos from the profile (avatars) of a vk user to Yandex.Disk cloud storage.
For photo titles, the number of likes is used, if the number of likes is the same, then the upload date is added.
Information on the saved photos is saved in a json file.*
